...
public class PcGui7
{
	...
	private final FcLog log = FcLog.getInstance();
	private McPoint point;
...
//		tabMain.setSelectedIndex(1);
		...
		btnLogin.addActionListener((e)->
		{
			...
			new Thread(()->
			{
				if(btnLogin.getText().equals("登录"))
				{
					...
					{
						frmMain.setTitle(frmMain.getTitle()+"["+(String)iptAcout.getSelectedItem()+"]["+action.arthur.info.name+"]");
						tray.setToolTip("["+(String)iptAcout.getSelectedItem()+"]["+action.arthur.info.name+"]");
					...
					for(DcArea a:action.arthur.areas)
					{
						if(a.name.equals(valExpArea.getText()))
						{
							try
							{
								...
								for(DcFloor f:FcUtil.reverse(a.floors))
	...
	public void ifoShow()
	{
		if(action.arthur != null)
		{
			valIfoName.setText(action.arthur.info.name);
			valIfoLevel.setText(action.arthur.info.level);
			valIfoCountry.setText(new String[]{"剑术之城","技巧之场","魔法之派"}[Integer.parseInt(action.arthur.info.country)-1]);
			valIfoIDUser.setText(action.arthur.info.uid);
			valIfoGoldCoin.setText(action.arthur.info.gold);
			valIfoMillionCost.setText(action.arthur.info.mc);
			valIfoFriendPoint.setText(action.arthur.info.frdPoint);
			valIfoGachaTicket.setText(action.arthur.info.gachatk);
			valIfoGachaPoint.setText(action.arthur.info.gachapt);
			valIfoFriendNow.setText(action.arthur.info.frdCount);
			valIfoFriendMax.setText(action.arthur.info.frdMax);
			valIfoFriendIvt.setText(action.arthur.info.frdInvitation);
			
			valExgauge.setText(action.arthur.info.exgauge+"%");
			prgExgauge.setBounds(70, 20, (int)(100*(Double.valueOf(action.arthur.info.exgauge)/100.0)), 20);
			
			if(action.arthur.info.hasFairy)
			{
				btnHaveFairy.setUI(new BEButtonUI().setNormalColor(BEButtonUI.NormalColor.red));
			...
			if(action.arthur.info.hasReward)
			{
				btnHaveReward.setUI(new BEButtonUI().setNormalColor(BEButtonUI.NormalColor.red));
			...
			for(DcItem i:action.arthur.items)
			{
				switch(i.iid)
				{
				case "1":
					valItmAP.setText("剩余"+i.count+"瓶"); break;
				case "2":
					valItmBC.setText("剩余"+i.count+"瓶"); break;
				case "111":
					valItmAPHalf.setText("剩余"+i.count+"瓶"); break;
				case "112":
					valItmBCHalf.setText("剩余"+i.count+"瓶"); break;
				}
			}
			
			if(point == null)
			{
				point = new McPoint(action.arthur.points, valAP, valBC, prgAP, prgBC);
				action.arthur.points.timer = point;
			}
