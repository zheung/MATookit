...
public class FcResCard
{
	...
	public static List<NDataCard> crtCard(int version) throws Exception 
	{
		...
		List<NDataCard> list = new ArrayList<>();
	//Read macFile
		...
		for(Iterator<?> i = r.elementIterator("card"); i.hasNext();)
	    {
	    	...
	    	NDataCardTemp dc = new NDataCardTemp();
			...
			dc.skill = new NDataCardSkillLess(e.element("skill_name").getStringValue(), e.element("skill_kana").getStringValue()
					, e.element("skill_description").getStringValue().replace("|"," "), e.element("skill_type").getStringValue());
	...
	public static List<NDataCard> anlCard(int version) throws Exception
	{
